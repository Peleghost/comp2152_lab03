...
while not combatStrength.isnumeric() or int(combatStrength) < 1 or int(combatStrength) > 6:
    print("Input must be a number between 1 and 6")
    combatStrength = input("Enter your combat Strength: (Number between 1-6) ")

combatStrength = int(combatStrength)

mCombatStrength = input("Enter monsters combat Strength: (Number between 1-6) ")

while not mCombatStrength.isnumeric() or int(mCombatStrength) < 1 or int(mCombatStrength) > 6:
    print("Input must be a number between 1 and 6")
    mCombatStrength = input("Enter your combat Strength: (Number between 1-6) ")
...
for j in range(1, 20, 2):
    if i == 11:
        print("Batlle Truce declared in round 11. Game Over!")

    roll = random.choice(diceOptions)
    roll2 = random.choice(diceOptions)
    
    combatStrength += roll
    mCombatStrength += roll2
    ...
    heroWeapon = weapons[roll]
    monsterWeapon = weapons[roll2]     

    print(f"Round {i}: Hero rolled {roll}, Monster rolled {roll2}.")
    i += 1
